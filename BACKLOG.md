...
- [~] **Rationalisation de la télémétrie post-prompt et volet déroulant mobile** : Indicateurs opérationnels en volet déroulant natif Telegram (`<blockquote expandable>`), vérité terrain via input tokens du tour LLM et sonde PTY, alertes visuelles ciblées (0 outil exécuté) et libellés normalisés en anglais ([Issue #9](https://github.com/Homeboyz-IT/agy-telegram-private/issues/9), [Spécifications](docs/specs/telemetry-metrics-rationalization.md)).
