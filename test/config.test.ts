...
test("loads safe defaults and allowlist", () => { const config = loadConfig(base); assert.deepEqual(config.telegram.allowedUserIds, ["123", "456"]); assert.equal(config.telegram.privateOnly, true); assert.equal(config.agy.mode, "plan"); assert.equal(config.agy.sandbox, true); assert.equal(config.agy.allowSandboxDisable, false); assert.equal(config.agy.effort, "high"); assert.ok(config.agy.allowedModels.includes("claude-sonnet-4-6")); });
